...
In case it wasn’t clear, the two variables `group` and `treatment` are redundant. Whereas the former is composed of names, the latter is the dummy-variable (0/1 coding) equivalent, for convenience. The main event was how we used the `rnorm()` function to simulate the normally-distributed values for `y`.
...
```{r}
tidy(updated_fit, prob = 0.95)
```

Well how about that? Despite knowing the "oracle" (some say "true") value is 0.5, our 95% credible intervals for $\beta_1$ did include zero (0) within their bounds! Though the posterior mean, 0.30, is still well away from zero, here we'd fail to reject $H_0$ at the conventional level. This is why we simulate.
...
We're more than half way there! It's time to do our first precision simulation.

## Simulate to determine precision
